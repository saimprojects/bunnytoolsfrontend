...
// src/pages/ProductDetail.jsx - Updated for Digital Products with Plans Decoding
import React, { useEffect, useState } from 'react';
import { useParams, Link } from 'react-router-dom';
import { 
  Download, 
...
// Helper function to decode plan code
const decodePlanCode = (code) => {
  if (!code || code.length < 5) return null;
  
  const typeCode = code.substring(0, 3);
  const quantity = parseInt(code.substring(3), 10);
...
const formatPlanDisplay = (plan) => {
  const decoded = decodePlanCode(plan.duration_code);
  if (!decoded) {
    return {
...
const ProductDetail = () => {
  ...
  const getPlanIcon = (index, plan) => {
    ...
    const icons = [CreditCard, Zap, Crown];
    return icons[index % icons.length];
  };

  // Rest of the component remains the same until the plan selection section...
                  ...
                  <div className="mt-6 bg-gradient-to-r from-purple-50 to-white rounded-xl p-6 border border-purple-100">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-gray-900">Rs. {selectedPlan.price}</div>
                        <div className="text-sm text-gray-500">
                          {formatPlanDisplay(selectedPlan).displayText} access
                        </div>
                      </div>
                    </div>

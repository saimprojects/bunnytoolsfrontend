...
function useScrollLockSection(totalVirtualScroll = 1200) {
  ...
  useEffect(() => {
    ...
    const shouldCapture = (delta) => {
      ...
      const shouldHold =
        metrics.inLockZone &&
        ((movingDown && hasRoomForward) || (movingUp && hasRoomBackward));
      const shouldReverseEnter =
        movingUp &&
        metrics.rect.top <= 0 &&
        metrics.rect.bottom >= window.innerHeight - startThreshold &&
        hasRoomBackward;
    ...
    const onScroll = () => {
      ...
      if (
        metrics.rect.top > 140 &&
        (currentProgressRef.current !== 0 || targetProgressRef.current !== 0)
      ) {
      ...
      if (
        metrics.rect.bottom < window.innerHeight - 140 &&
        (currentProgressRef.current !== 1 || targetProgressRef.current !== 1)
      ) {
  ...
  const wrapperStyle = useMemo(
    () => ({
      position: 'relative',
      height: `calc(100vh + ${Math.round(Math.max(1, totalVirtualScroll) * 0.54)}px)`,
    }),
    [totalVirtualScroll]
  );
...
function ScrollBar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 h-[3px] z-[60] pointer-events-none">
      <div
        className="h-full transition-[width_.1s]"
        style={{
          width: `${p}%`,
          background: T.gP,
          boxShadow: `0 0 10px ${T.p}`,
        }}
      />
    </div>
  );
...
function Reveal({ children, delay = 0, className = '' }) {
  ...
  return (
    <div
      ...
      className={className}
      style={{
        transform: iv ? 'translateY(0)' : 'translateY(48px)',
        opacity: iv ? 1 : 0,
...
function ProgressRing({ progress }) {
  ...
  return (
    ...
    >
      ...
      >
        <svg width="52" height="52" className="progress-ring absolute">
          <circle cx="26" cy="26" r={r} fill="none" stroke="rgba(124,58,237,.2)" strokeWidth="3" />
          <circle
            cx="26"
            cy="26"
            r={r}
        ...
        <span className="font-mono text-[10px] text-purple-300">{Math.round(progress * 100)}%</span>
      </div>
    </div>
  );
}
...
function ScrollHint({ visible, progress }) {
  ...
  return (
    ...
    >
      ...
      <div className="w-5 h-8 rounded-full border border-purple-500/40 flex items-start justify-center p-1">
        <div
          className="w-1 h-2 rounded-full bg-purple-400"
          style={{ animation: 'float-y 1.5s ease-in-out infinite' }}
        />
      </div>
    </div>
  );
    ...
    <div ref={sectionRef} className="scroll-lock-wrapper" style={{ ...wrapperStyle, background: T.dd }}>
      <div className="scroll-lock-sticky">
        ...
        <div
          ...
          style={{
            opacity: 0.14 + progress * 0.2,
            background: `radial-gradient(circle at 50% 50%, rgba(167,139,250,${0.16 + progress * 0.14}), transparent 42%)`,
            transform: `scale(${0.86 + progress * 0.16})`,
          }}
        />
        ...
        <div
          ...
          style={{
            ...
            transform: `translate(-50%,-50%) rotate(${progress * 180}deg) scale(${0.9 + progress * 0.08})`,
          ...
          <div className="max-w-5xl mx-auto">
            <div
              style={{
                ...
                transform: `translateY(${(1 - Math.min(1, progress * 5)) * 20}px)`,
              }}
              className="mb-3"
            >
              <span className="font-mono text-[10px] tracking-widest uppercase text-purple-400 flex items-center gap-2">
              ...
              {lines.map((line, i) => {
                ...
                return (
                  <div key={i} style={{ overflow: 'hidden' }}>
                    <p
                      className="font-display text-2xl md:text-4xl lg:text-5xl xl:text-6xl font-bold leading-tight"
                      style={{
                        transform: `translateY(${(1 - lineProgress) * 70}px)`,
                        opacity: lineProgress,
                      }}
                    >
                      {words.map((w, wi) => {
                        const isHL = line.hl.some(
                          (h) =>
                            w === h ||
                            w.replace(/[.,!?]/g, '') === h.replace(/[.,!?]/g, '')
                        );

                        return (
                          <span
                            key={wi}
                            className="inline-block mr-[.22em]"
                            style={{
                              color:
                                isHL && lineProgress > 0.35
                                  ? T.pl
                                  : `rgba(255,255,255,${0.12 + lineProgress * 0.88})`,
                            }}
                          >
                            {w}
                          </span>
            ...
            <div className="mt-4 md:mt-6 flex items-center gap-4">
              <div className="flex-1 h-px bg-white/5 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${progress * 100}%`, background: T.gP }}
                />
              </div>
              <span className="font-mono text-xs text-purple-400">{Math.round(progress * 100)}%</span>
            </div>
          </div>
        </div>

        <ProgressRing progress={progress} />
        <ScrollHint visible progress={progress} />
      </div>
    </div>
  );
}
  ...
  const items = [
    {
      ...
      desc: 'Get access immediately after purchase. Zero waiting, zero delays. Your credentials land in your inbox in under 60 seconds.',
      color: '#10b981',
      stat: '<1min',
      sl: 'Avg Delivery',
    },
    {
      ...
      desc: 'Pay once. Use forever. All future updates are included at no extra cost.',
      color: T.p,
      stat: '∞',
      sl: 'Free Updates',
    },
    {
      ...
      desc: 'Dedicated support team always online. Get help via live chat, email, or WhatsApp.',
      color: T.y,
      stat: '24/7',
      sl: 'Live Support',
    },
    {
      icon: <Cpu size={28} />,
      title: 'Premium Tool Quality',
      desc: 'Every subscription is rigorously tested and verified before it reaches you.',
      color: '#ec4899',
      stat: '98%',
      sl: 'Satisfaction',
    },
      ...
      <section className="py-16 px-4 bg-purple-50">
        <div className="mb-8 text-center">
          ...
          <h2 className="font-display text-2xl font-bold text-gray-900">
            Built for <span className="text-gradient-p">serious creators.</span>
          </h2>
        </div>

        <div className="space-y-4">
          {items.map((item, i) => (
            <div key={i} className="p-6 rounded-2xl glass-light">
              <div
              ...
              <h3 className="font-display font-bold mb-2 text-gray-900">{item.title}</h3>
              <p className="text-sm text-gray-600 mb-4">{item.desc}</p>
              <div className="text-2xl font-bold" style={{ color: item.color }}>
                {item.stat}
              </div>
              <div className="font-mono text-[10px] text-gray-400 mt-1">{item.sl}</div>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="w-full px-8 relative z-10">
          <div className="max-w-7xl mx-auto mb-6">
            ...
            <div className="flex items-end justify-between">
              ...
              <span className="font-mono text-sm text-purple-400 hidden md:block">
                {Math.round(progress * 100)}% explored
              </span>
            </div>

            <div className="mt-4 h-[2px] rounded-full overflow-hidden bg-purple-200/60">
              <div
                className="h-full rounded-full"
                style={{
          ...
          {(() => {
            ...
            const maxShift = Math.max(0, totalW - (ww - 80));
            const shift = progress * maxShift;

            return (
              <>
                <div className="overflow-hidden">
                  <div className="flex gap-6" style={{ transform: `translateX(-${shift}px)` }}>
                    {items.map((item, i) => {
                      const ip = Math.max(0, Math.min(1, (progress - i * 0.14) / 0.22));

                      return (
                        <div
                          key={i}
                          className="flex-shrink-0 w-[480px] p-8 rounded-3xl glass-light group relative overflow-hidden"
                          style={{
                            opacity: 0.12 + ip * 0.88,
                            transform: `scale(${0.88 + ip * 0.12}) translateY(${(1 - ip) * 30}px)`,
                          }}
                        >
                          <div
                            className="absolute top-0 right-0 w-52 h-52 pointer-events-none"
                            style={{ background: `radial-gradient(circle,${item.color}12,transparent 65%)` }}
                          />
                          <div
                          ...
                          <h3 className="font-display text-xl font-bold mb-3 text-gray-900">{item.title}</h3>
                          <p className="text-sm text-gray-600 mb-8 leading-relaxed">{item.desc}</p>
                          <div className="flex items-end justify-between">
                            <div>
                              <div className="font-display text-4xl font-bold" style={{ color: item.color }}>
                                {item.stat}
                              </div>
                              <div className="font-mono text-[10px] text-gray-400 mt-1">{item.sl}</div>
                            </div>
                ...
                <div className="flex justify-center gap-2 mt-5">
                  {items.map((_, i) => {
                    const active = progress > i / items.length && progress <= (i + 1) / items.length;
                    return (
                      <div
                        key={i}
                        ...
                        style={{
                          width: active ? 32 : 8,
                          background: active ? T.p : 'rgba(124,58,237,.2)',
                        }}
                      />
                    );
                  })}
...
function PinnedTextReveal() {
  ...
  const text =
    "Bunny Tools isn't just another reseller. It's the smartest way to access the digital tools you need at prices that actually make sense for creators, freelancers, and businesses that refuse to overpay.";
    ...
    <div ref={sectionRef} className="scroll-lock-wrapper" style={{ ...wrapperStyle, background: '#f0ebff' }}>
      <div className="scroll-lock-sticky">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-8 relative z-10">
          <div className="mb-4" style={{ opacity: Math.min(1, progress * 4) }}>
            <span className="font-mono text-[10px] tracking-widest uppercase text-purple-500">
              About Us
            </span>
          </div>

          <p className="font-display text-2xl md:text-4xl lg:text-5xl font-bold leading-relaxed">
            {text.split(' ').map((w, i, arr) => {
              const wp = Math.max(0, Math.min(1, (progress - (i / Math.max(1, arr.length - 1)) * 0.82) / 0.09));
              ...
              return (
                <span
                  key={i}
                  className="inline-block mr-[.25em]"
                  style={{
                    opacity: 0.07 + wp * 0.93,
                    transform: `translateY(${(1 - wp) * 20}px)`,
                    filter: `blur(${(1 - wp) * 1.2}px)`,
                    color: isHL && wp > 0.5 ? T.p : '#1e1b4b',
                  }}
                >
                  {w}
                </span>
          ...
          <div className="mt-5 h-[3px] w-full rounded-full overflow-hidden bg-purple-200/70">
            <div
              className="h-full rounded-full"
              style={{
                width: `${progress * 100}%`,
                background: 'linear-gradient(90deg,#7c3aed,#a855f7,#f59e0b)',
                boxShadow: '0 0 20px rgba(124,58,237,.22)',
              }}
            />
          </div>
        </div>
  ...
  const nums = [
    {
      ...
      desc: 'Creators and businesses trust Bunny Tools globally.',
      icon: <Users size={22} />,
    },
    {
      val: 50,
      sfx: '+',
      label: 'Premium Tools',
      desc: 'Curated collection of top-tier digital subscriptions.',
    ...
    },
    {
      ...
      desc: 'Overwhelmingly positive feedback every month.',
      icon: <Star size={22} />,
    },
    {
      val: 30,
      ...
      desc: 'Risk-free purchases with full, no-questions refund.',
        ...
        <div className="w-full px-4 sm:px-8 relative z-10">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-8">
              <h2
                className="font-display text-3xl md:text-5xl font-bold text-gray-900"
                style={{
                  opacity: Math.min(1, progress * 3),
                  transform: `translateY(${(1 - Math.min(1, progress * 3)) * 32}px)`,
                }}
              >
                Numbers that <span className="text-gradient-p">speak for us.</span>
              </h2>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {nums.map((n, i) => {
                const ip = Math.max(0, Math.min(1, (progress - 0.03 - i * 0.09) / 0.16));
                const counted = (1 - Math.pow(1 - ip, 4)) * n.val;

                return (
                  <div
                    key={i}
                    className="text-center group"
                    style={{ opacity: ip, transform: `translateY(${(1 - ip) * 38}px)` }}
                  >
                    <div
                      className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4"
                      style={{ background: `${T.p}12`, color: T.p }}
                    >
                      {n.icon}
                    </div>
                    ...
                    <h3 className="font-display font-bold mt-3 mb-2 text-gray-900">{n.label}</h3>
...
function FeatureCard({ icon: Icon, title, description, index }) {
  ...
  return (
    <div
      ...
      style={{
        background: hover ? '#160d2e' : 'rgba(255,255,255,.55)',
        backdropFilter: 'blur(20px)',
        border: `1px solid ${hover ? `${T.p}30` : 'rgba(124,58,237,.1)'}`,
        transform: iv ? 'translateY(0)' : 'translateY(50px)',
        opacity: iv ? 1 : 0,
        transition: `all .7s cubic-bezier(.16,1,.3,1) ${index * 0.04}s`,
      }}
    ...
    >
      <div
        ...
        style={{
          background: hover ? `${T.p}25` : `${T.p}12`,
          color: T.p,
      ...
      <p
        ...
        style={{ color: hover ? '#94a3b8' : '#64748b' }}
      >
        {description}
      </p>
    </div>
  ...
  return (
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
        <Reveal>
          <div className="text-center mb-12">
            <span className="font-mono text-xs text-purple-500 uppercase tracking-widest">Trust Layer</span>
            <h2 className="font-display text-3xl md:text-5xl font-bold text-gray-900 mt-3">
              Reliable from first click to final delivery
            </h2>
          </div>
        </Reveal>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {trust.map((t, i) => (
            <Reveal key={i} delay={i * 0.05}>
              <div className="p-6 rounded-2xl border border-purple-100 bg-white shadow-sm">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4 bg-purple-100 text-purple-700">
                  <t.icon size={20} />
                </div>
                <h3 className="font-display text-lg font-bold text-gray-900 mb-2">{t.title}</h3>
  ...
  return (
    <section className="py-20 bg-[#faf7ff]">
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
        <Reveal>
          <div className="text-center mb-12">
            <span className="font-mono text-xs text-purple-500 uppercase tracking-widest">How it works</span>
            <h2 className="font-display text-3xl md:text-5xl font-bold text-gray-900 mt-3">
              Simple process. Fast result.
            </h2>
          </div>
        </Reveal>
          ...
          {steps.map((s, i) => (
            <Reveal key={i} delay={i * 0.06}>
              <div className="glass-light rounded-3xl p-7 h-full">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5 bg-purple-100 text-purple-700">
                  <s.icon size={22} />
                </div>
...
function StatsSection() {
  ...
  return (
    <section
      className="py-20"
      style={{ background: 'linear-gradient(90deg,#6d28d9 0%, #7c3aed 50%, #8b5cf6 100%)' }}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {stats.map((stat, index) => (
  ...
  return (
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
        <Reveal>
          <div className="text-center mb-12">
            <span className="font-mono text-xs text-purple-500 uppercase tracking-widest">Why us</span>
            <h2 className="font-display text-3xl md:text-5xl font-bold text-gray-900 mt-3">
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {reasons.map((r, i) => (
            <Reveal key={i} delay={i * 0.05}>
              <div className="rounded-3xl border border-purple-100 p-7 bg-white shadow-sm h-full">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5 bg-purple-100 text-purple-700">
                  <r.icon size={22} />
                </div>
                <h3 className="font-display text-xl font-bold text-gray-900 mb-3">{r.title}</h3>
  ...
  const faqs = [
    {
      ...
      a: 'Most orders are processed very quickly and shared soon after purchase.',
    },
    {
      q: 'Are these subscriptions verified?',
      a: 'Yes. Quality and access are checked before delivery.',
    },
    {
      q: 'What if I need help after purchase?',
      a: 'Support stays available, so you are not left alone after checkout.',
    },
  ];
  ...
  return (
    <section className="py-20 bg-[#faf7ff]">
      <div className="max-w-5xl mx-auto px-4 sm:px-8">
        <Reveal>
          <div className="text-center mb-12">
            <span className="font-mono text-xs text-purple-500 uppercase tracking-widest">FAQ</span>
            <h2 className="font-display text-3xl md:text-5xl font-bold text-gray-900 mt-3">
          ...
          {faqs.map((f, i) => (
            <Reveal key={i} delay={i * 0.04}>
              <div className="rounded-2xl border border-purple-100 bg-white p-6">
                <h3 className="font-display text-lg font-bold text-gray-900 mb-2">{f.q}</h3>
...
function CTASection() {
  return (
    <section className="py-24 bg-[#160d2e] relative overflow-hidden">
      <Blob color="rgba(124,58,237,.10)" size={520} style={{ top: '-10%', left: '-5%' }} />
      <Blob color="rgba(245,158,11,.08)" size={420} style={{ bottom: '-10%', right: '-5%' }} />

      <div className="max-w-6xl mx-auto px-4 sm:px-8 relative z-10">
        <Reveal>
          <div className="rounded-[32px] border border-white/10 bg-white/5 backdrop-blur-xl p-8 md:p-12 text-center">
            ...
            <h2 className="font-display text-3xl md:text-5xl font-bold text-white mb-4">
              ...
              <br />
              <span className="text-gradient-p">without overpaying.</span>
            </h2>

            <p className="text-white/65 max-w-2xl mx-auto mb-8 leading-relaxed">
              Browse premium subscriptions, pick what fits your work, and get access fast.
            </p>

            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link
                to="/products"
                className="group inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-semibold text-base text-white transition-all duration-300 relative overflow-hidden"
                style={{ background: T.gP, boxShadow: '0 14px 40px rgba(124,58,237,.45)' }}
              >
              ...
              <Link
                to="/contact"
                className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-semibold text-base bg-white/10 backdrop-blur-sm border border-white/20 text-white hover:bg-white/15 transition-all duration-300"
              >
              ...
              <div className="flex items-center justify-center gap-6 mt-10 pt-10 border-t border-white/10">
                ...
                <div className="text-left">
                  <div className="flex gap-0.5 mb-1">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} size={12} fill={T.y} stroke="none" />
                    ))}
                  </div>
                  <p className="font-mono text-[10px] text-gray-400">Loved by 1,000+ customers</p>
                </div>
              </div>
            </Reveal>
          </div>
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const products = await getProducts();
        setFeaturedSoftwares(products.slice(0, 4));
        setTrendingSoftwares(products.slice(4, 8));
      } catch (e) {
        console.error(e);
      } finally {
        setLoading(false);
  ...
  const features = [
    {
      ...
      description: 'Get access immediately after purchase. Credentials in your inbox in under 60 seconds.',
    },
    {
      icon: Shield,
      title: 'Lifetime License',
      description: 'One-time payment. All future updates included forever at no extra cost.',
    },
    {
      icon: Zap,
      title: 'Premium Support',
      description: '24/7 dedicated technical support via chat, email, and WhatsApp.',
    },
    {
      icon: Cpu,
      title: 'Always Updated',
      description: 'Latest features always included. We keep your tools current automatically.',
    },
    {
      icon: Lock,
      title: 'Private & Secure',
      description: 'Dedicated accounts assigned only to you. Complete privacy, zero sharing.',
    },
    {
      icon: Globe,
      title: 'Global Access',
      description: 'Use your tools from anywhere in the world, on any device, anytime.',
    },
  ];
    ...
    <div className="font-body overflow-x-hidden bg-white">
          ...
          {[...Array(3)].map((_, si) => (
            <React.Fragment key={si}>
              {marqueeItems.map((t, i) => (
                <span
                  key={`${si}-${i}`}
                  className="mx-4 md:mx-6 font-mono text-[10px] md:text-xs font-medium tracking-wide"
                  style={{ color: t === '✦' ? '#a78bfa' : '#cbd5e1' }}
                >
                  {t}
                </span>
              ))}
      ...
      <section className="py-20 relative overflow-hidden bg-purple-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 relative z-10">
          <div className="text-center mb-12">
            <Reveal>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-4 bg-purple-500/10 border border-purple-500/20">
            ...
            <Reveal delay={0.06}>
              <h2 className="font-display text-3xl md:text-5xl font-bold text-gray-900 mb-3">
                All the tools you need to
                <br />
                <span className="text-gradient-p">dominate your work.</span>
              </h2>
            </Reveal>
          </div>
      ...
      {/* {trendingSoftwares.length > 0 && (
        <HomeCards
          ...
          subtitle="Fast-moving hot picks with the same updated compact stack and improved mobile responsiveness."
          products={trendingSoftwares}
          loading={loading}
          viewAllLink="/products"
          featuredMode={false}
          trendingMode={true}
